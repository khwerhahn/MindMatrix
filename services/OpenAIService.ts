	...
	async createEmbeddings(chunks: string[]): Promise<EmbeddingResponse[]> {
		if (!this.client) {
			...
			return chunks.map(() => ({
				...
				model: "text-embedding-ada-002", // Default model to maintain output consistency
			}));
		}
		...
		for (let i = 0; i < chunks.length; i++) {
			try {
				...
				// Explicitly set the model to `text-embedding-ada-002`
				const response = await this.client.embeddings.create({
					model: "text-embedding-ada-002", // Use the correct model
					input: chunks[i],
